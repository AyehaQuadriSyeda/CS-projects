...
// This file is used for testing graph.h. It is not using Google Test Framework.
// You can still use gtest, but it is not required (because we will not be
// grading it).
//
//
...
void buildGraph(string filename, graph<string, int> &G) {
  ...
  while (src != "#") {
    ...
    if (!G.addEdge(src, dest, weight))
      cout << "**Error: unable to add edge (" << src << "," << dest << ","
           << weight << "), why not?" << endl;
